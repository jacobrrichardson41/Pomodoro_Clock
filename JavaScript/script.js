...
function Clock() {
    var startTime = 60, //Starting a value for our timer
    currentTime = 60, //Current time for our timer 
    sessionTime = 60, //Length of a break in seconds
    breakTime = 60, // Length of a break in seconds 
    ...
    timer; //Reference to the interval that we set up to make the time run
    ...
    //Toggle the clock between started and paused
    ...
    //Subtract one second from the currentTime, display the new currentTime, and when time runs out, alternate between session and break
    this.stepDown = function() {
        console.log("StepDown called");
        if (currentTime > 0) {
            currentTime--;
            this.displayCurrentTime();
            if (currentTime ===0) {
